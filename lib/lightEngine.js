...
import { hexToRgb, hslToRgb, pickColor, rgba } from './palette';
import { drawBackdrop, drawHexWall } from './backdrop';
import { drawRetroHexPars } from './retroHex';
import { drawSingleHexPars } from './megaHex';
...
function buildLayout(w, h) {
  // Fixture hardware is sized off `s`, not the raw height: on a tall phone the
  // stage is narrow, and scaling lenses and pools by height alone blows them up.
  const s = Math.min(h, w * 0.62);
  const portrait = w / h < 1.05;

  const trussY = h * (portrait ? 0.07 : 0.085);
  const wallBottom = h * (portrait ? 0.56 : 0.63);
  const heads = [];
  ...
  const arch = { cx: w * 0.5, cy: wallBottom + h * 0.02, r: Math.min(h * 0.5, w * 0.54) };
  const pars = [];
  const parCount = 11;
  ...
  // Ground smoke: heavier and slower than haze, and it never leaves the deck.
  // Every puff drifts the same way — a fogger blows across a stage, it does not
  // scatter — with a per-puff lane and rate so the layer still has depth.
  const smoke = [];
  for (let i = 0; i < 22; i++) {
    smoke.push({
      x: rnd(),
  ...
  return {
    w, h, s, portrait, trussY, wallBottom,
    heads, arch, pars, classicPars, blinders, strobes, lasers, crowd, fog, smoke,
  };
}
...
export class Rig {
  ...
  frame(dt, scene, theme, audio) {
    ...
    // Back wall first: the LED surface everything else is lit in front of.
    if (!scene.blackout) {
      if (scene.fixtures.hexPanel) drawHexWall(ctx, L, theme, scene, t, kick, this.clock.bar);
      drawBackdrop(ctx, L, theme, scene, t, kick, this.clock.bar);
      // Standing hardware in front of the wall, behind the beams. The single
      // head is furthest upstage, so it goes down first.
      if (scene.fixtures.singleHex) drawSingleHexPars(ctx, L, theme, scene, t, kick, this.clock.bar);
      if (scene.fixtures.retroHex) drawRetroHexPars(ctx, L, theme, scene, t, kick, this.clock.bar);
    }
    ...
    // After the beams, so the layer reads as smoke being lit rather than a
    // grey wash sitting on top of the stage.
    if (scene.fixtures.smoke && scene.smokeDensity > 0) {
      this.#smoke(ctx, L, t, scene.smokeDensity / 100, (scene.smokeSpeed ?? 40) / 100, theme, scene, master);
    }
    ctx.globalCompositeOperation = 'source-over';
  ...
  /**
   * Low smoke. Where the hazer fills the whole room so beams read in the air,
   * the smoke machine dumps a heavy layer that hugs the deck: it sits on the
   * floor line, rolls sideways, and only creeps up when the kick hits.
   */
  #smoke(ctx, L, t, density, speed, theme, scene, master) {
    const { w, h } = L;
    // The deck: from the floor line back there, to where the crowd starts.
    const top = L.wallBottom;
    const bottom = h * 0.86;
    const depth = bottom - top;
    const band = depth * (0.5 + density * 0.7); // how deep the layer lies

    ctx.save();
    // Clipped to the deck, so it can never fog the back wall or the truss.
    ctx.beginPath();
    ctx.rect(0, top - depth * 0.2, w, depth * 1.5);
    ctx.clip();
    ctx.globalCompositeOperation = 'lighter';

    const tint = pickColor(theme, scene.colorMode, 0, t * 0.25, this.clock.bar);
    const base = [140 + tint[0] * 0.3, 146 + tint[1] * 0.28, 158 + tint[2] * 0.25];
    // Screens per second. Deliberately not beat-linked: smoke keeps rolling at
    // whatever the machine and the room are doing, it doesn't pump on the kick.
    const drift = 0.004 + speed * 0.09;

    L.smoke.forEach((p) => {
      const rate = drift * (0.7 + p.lane * 0.6);
      const x = (p.x + t * rate) % 1.4;
      const px = x * w * 1.3 - w * 0.15;

      // Each puff keeps its own height on the deck; the only vertical movement
      // is a slow wander, far below the rate that would read as bouncing.
      const wander = Math.sin(t * 0.07 + p.ph) * 0.04;
      const y = bottom - band * (0.08 + p.y * 0.8 + wander);

      const r = p.r * w * (0.4 + density * 0.5);
      const a = density * 0.18 * (0.35 + master * 0.65);

      ctx.save();
      ctx.translate(px, y);
      // Squashed hard: smoke on a stage spreads far wider than it climbs.
      ctx.scale(1, 0.34);
      const g = ctx.createRadialGradient(0, 0, 0, 0, 0, r);
      g.addColorStop(0, rgba(base, a));
      g.addColorStop(0.5, rgba(base, a * 0.42));
      g.addColorStop(1, 'rgba(0,0,0,0)');
      ctx.fillStyle = g;
      ctx.beginPath();
      ctx.arc(0, 0, r, 0, TAU);
  ...
  #ledRamp(ctx, L, theme, scene, t, master, kick) {
    ...
    for (let r = 0; r < rings; r++) {
      ...
      ctx.lineWidth = Math.max(6, L.s * 0.022);
      ctx.stroke();
    }
    ...
    L.pars.forEach((p) => {
      ...
      drawPool(ctx, p.x, p.y, L.s * 0.1, L.s * 0.1, c, a);
      ctx.beginPath();
      ctx.arc(p.x, p.y, Math.max(2.5, L.s * 0.007), 0, TAU);
  ...
  #classicPars(ctx, L, theme, scene, t, master, haze) {
    L.classicPars.forEach((p) => {
      ...
      drawBeam(ctx, p, Math.PI - angle, L.h * 0.62, L.s * 0.012, L.s * 0.16, warm, a, 0.85);
      drawPool(ctx, p.x, p.y, L.s * 0.06, L.s * 0.03, warm, master * breathe * 0.5);
    });
  }
  ...
  #movingHeads(ctx, L, theme, scene, t, master, haze, kick, speed) {
    ...
    L.heads.forEach((head) => {
      ...
      drawPool(ctx, head.x, head.y, L.s * 0.035, L.s * 0.035, c, master * 0.5);
    });
  }
  ...
  #lasers(ctx, L, theme, scene, t, master, haze, kick) {
    ...
    const len = Math.max(L.w * 0.72, L.h * 0.5);
    ...
    L.lasers.forEach((em) => {
      ...
      for (let i = 0; i < density; i++) {
        ...
        ctx.lineWidth = Math.max(2.5, L.s * 0.005);
        ctx.stroke();
      ...
      drawPool(ctx, em.x, em.y, L.s * 0.05, L.s * 0.05, [255, 255, 255], master * 0.3);
    });
  }
  ...
  #blinders(ctx, L, scene, t, master, clock) {
    ...
    L.blinders.forEach((b) => {
      ...
      for (let cell = 0; cell < 4; cell++) {
        const cx = b.x + (cell - 1.5) * L.s * 0.028;
        drawPool(ctx, cx, b.y, L.s * 0.045, L.s * 0.045, c, master * env);
      }
    });
  }
  ...
  #strobe(ctx, L, scene, t, master) {
    ...
    ctx.fillRect(0, 0, L.w, L.h);
    L.strobes.forEach((s) => drawPool(ctx, s.x, s.y, L.s * 0.06, L.s * 0.06, [255, 255, 255], master));
  }
  ...
  #structure(ctx, L, theme, scene, master) {
    ...
    bodies.forEach((p) => {
      const bw = p.kind === 'blinder' ? L.s * 0.055 : L.s * 0.026;
      const bh = L.s * 0.03;
    ...
    const bx = w * 0.36;
    const bw = w * 0.28;
    const by = L.wallBottom + L.s * 0.06;
    const bhh = L.s * 0.16;
    ...
    ctx.fillStyle = bg;
    ctx.fillRect(bx + 4, by + 4, bw - 8, L.s * 0.012);
