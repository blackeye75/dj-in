...
function buildLayout(w, h) {
  ...
  for (let i = 0; i < 20; i++) {
    smoke.push({
      ...
      y: rnd(),
      r: 0.1 + rnd() * 0.17,
      sp: (0.006 + rnd() * 0.022) * (rnd() > 0.5 ? 1 : -1),
      ph: rnd() * TAU,
      lift: 0.5 + rnd() * 0.9,
    });
  }
...
export class Rig {
  ...
  frame(dt, scene, theme, audio) {
    ...
    if (scene.fixtures.smoke && scene.smokeDensity > 0) {
      this.#smoke(ctx, L, t, scene.smokeDensity / 100, theme, scene, master, kick);
    }
    ctx.globalCompositeOperation = 'source-over';
  ...
  #smoke(ctx, L, t, density, theme, scene, master, kick) {
    ...
    L.smoke.forEach((p) => {
      const x = (((p.x + t * p.sp) % 1.4) + 1.4) % 1.4;
      const bob = Math.sin(t * 0.5 + p.ph) * 0.08 + kick * p.lift * 0.16;
      const y = bottom - band * (p.y * 0.8 + bob);
      const r = p.r * w * (0.4 + density * 0.5);
      const a = density * 0.18 * (0.35 + master * 0.65);
      const px = x * w * 1.2 - w * 0.1;
