 ...
 * rings of LEDs around a high-output centre. The 7-head cluster owns the floor
 * line, so this one is flown off the truss instead and aimed *down* over the
 * booth — the two fixtures stop fighting for the same patch of stage, and the
 * downward throw lands a pool of light on the DJ rather than into their eyes.
 *
...
function drawUnit(ctx, u, L, theme, scene, t, kick, bar, cache) {
  ...
  // Yoke: the drop from the truss it hangs on.
  ctx.save();
  ctx.strokeStyle = 'rgba(150,162,184,0.3)';
  ctx.lineWidth = Math.max(2, u.r * 0.08);
  ctx.beginPath();
  ctx.moveTo(u.x, L.trussY);
  ctx.lineTo(u.x, u.y - u.r * 0.84);
  ctx.stroke();
  ctx.restore();
  ...
  // The halo: a wide throw down onto the booth and the floor in front of it.
  // It stops at the stage floor rather than running off the bottom of the frame.
  if (halo > 0.02 && mode !== 'strobe') {
    const c = baseColour();
    const floor = L.h * 0.9;
    const reach = Math.min(u.r * (7 + halo * 9), floor - u.y);
    const spread = u.r * (2.2 + halo * 3.4);
    ...
    ctx.lineTo(u.x - spread, u.y + reach);
    ctx.lineTo(u.x + spread, u.y + reach);
    ctx.lineTo(u.x + u.r * 1.1, u.y);
    ctx.closePath();
    ctx.fill();

    // Where the throw lands: a soft pool, squashed by the viewing angle.
    ctx.save();
    ctx.translate(u.x, u.y + reach);
    ctx.scale(1, 0.26);
    // Built after the transform, so the gradient is squashed with the ellipse.
    const pg = ctx.createRadialGradient(0, 0, 0, 0, 0, spread);
    pg.addColorStop(0, rgba(c, out * halo * 0.22));
...
export function drawSingleHexPars(ctx, L, theme, scene, t, kick, bar) {
  ...
  // Flown off the truss, clear of the strobe and blinder boxes hung on it, so
  // the floor line is left to the 7-head cluster.
  const y = L.trussY + L.h * 0.075 + r * 1.15;
  const units =
    (scene.singleRig || 'single') === 'pair'
      ? [
          { x: L.w * 0.31, y, r },
          { x: L.w * 0.69, y, r },
        ]
  ...
  units.forEach((u) => drawUnit(ctx, u, L, theme, scene, t, kick, bar, L._mega));
}
