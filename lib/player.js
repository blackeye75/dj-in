...
/** Distinguishes each engine's player host; see #ensureYouTube. */
let engineSeq = 0;
...
export class PlayerEngine {
   ...
   */
    ...
    this.ytPromise = (async () => {
      ...
      // A fresh host every time, under an id unique to this engine.
      //
      // The host lives on document.body, outside React, so a client-side route
      // change (the Control panel link and back) leaves it behind. Once a
      // player has been built on a div the API replaces that div with an
      // <iframe> of the same id — so a later engine that reuses the id attaches
      // a SECOND player to the first one's frame. Both stay live, the dead
      // engine's handlers keep firing into a torn-down instance, and the old
      // video carries on playing under whatever the new one starts.
  ...
  destroy() {
    ...
    // Tear the embedded player down properly. Without this it survives the
    // unmount still playing, with its events wired to this dead engine.
    if (this.yt && typeof this.yt.destroy === 'function') {
      try {
        this.yt.destroy();
      } catch {
        /* already gone */
